...
// Check connection
    ...
    $sql = 'UPDATE events SET datemanu = "'.$_POST['datemanu'].'", photo = "'.$_POST['photo'].'", titre = "'.$_POST['titre'].'", texte = "'.$_POST['texte'].'", dates = "'.$_POST['dates'].'"  WHERE id="'.$_POST['id'].'"';
    if ($conn->query($sql) === TRUE) {
      $confirm= "<script>alert('Modification(s) effectuée(s) !".$_POST['id']."');
      window.location.href='admin3event.php'
      </script> ";
    } else {
      $confirm= "<script>alert('Erreur, contactez nous.');
      window.location.href='admin3event.php'</script>";
    }
      $conn->close();
    }
    echo $confirm;
